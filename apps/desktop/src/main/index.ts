...
function registerBuiltInTools(modelGateway: ModelGateway): void {
  ...
  modelGateway.registerTool({
    ...
    function: {
      name: 'create_file',
      description: 'Tworzy nowy plik tekstowy w workspace i zapisuje do niego zawartość. Używaj tylko dla nowych plików, nie do nadpisywania istniejących. Jeśli podano tylko nazwę pliku, ścieżka zostanie rozwiązana względem projektu wskazanego przez inne pliki lub nazwę projektu w rozmowie.',
      parameters: {
        type: 'object',
        properties: {
          filePath: { type: 'string', description: 'Ścieżka nowego pliku. Może być bezwzględna, względna względem aktywnego workspace albo sama nazwa pliku.' },
          content: { type: 'string', description: 'Treść UTF-8 zapisywana do nowego pliku.' }
        },
        required: ['filePath'],
        additionalProperties: false
      }
...
function inferCreateFileContent(filePath: string): string {
  ...
  if (extension === 'yaml' || extension === 'yml') {
    return '# Dodaj konfigurację YAML.\n';
  }
  return '';
}
...
function hasWriteIntent(lowerText: string): boolean {
  return lowerText.includes('zapisz')
    || lowerText.includes('zapisuję')
    || lowerText.includes('zapisano')
    || lowerText.includes('docelowa ścieżka')
    || lowerText.includes('potwierdź')
...
function hasTargetWriteMarker(text: string): boolean {
  return text.includes('zapisz do pliku')
    ...
    || text.includes('docelowa ścieżka')
    || text.includes('lokalizację');
}
...
function pickBestMatch(matches: readonly string[]): string | null {
  // Preferuj plik bezpośrednio w katalogu głównym projektu (najkrótsza ścieżka),
  // z pominięciem katalogów cache/build/test.
...
async function executeReadFileTool(toolCall: ToolCall, context?: ToolExecutionContext): Promise<string> {
  ...
  if (Array.isArray(resolvedPath)) {
    return JSON.stringify({
      status: 'error',
      code: 'AMBIGUOUS_FILE',
      message: `Znaleziono kilka plików o nazwie lub ścieżce pasującej do żądania: ${resolvedPath.join(', ')}`,
...
function noSearchResultsResponse(query: SearchQuery): string {
  return JSON.stringify({
    ...
    query,
    results: [],
    message: 'Nie znaleziono wyników wyszukiwania w aktywnym workspace.'
  });
}
...
function fileSearchFallbackResponse(query: SearchQuery, filePath: string, content: string, encoding: string): string {
  return JSON.stringify({
    ...
    content,
    encoding,
    message: 'Wyszukiwanie tekstowe nie zwróciło wyników; zwracam zawartość najlepiej pasującego pliku.'
  });
}
...
function fileMatchesWithoutContentResponse(query: SearchQuery, matches: readonly string[]): string {
  return JSON.stringify({
    ...
    message: 'Znaleziono pasujące pliki, ale nie udało się odczytać zawartości.'
  });
}
...
async function executeCreateFileTool(toolCall: ToolCall, context?: ToolExecutionContext): Promise<string> {
  ...
  if (!isSingleResolvedPath(resolvedPath)) {
    return JSON.stringify({
      status: 'error',
      code: 'WRITE_CONFLICT',
      message: `Ścieżka jest niejednoznaczna: ${resolvedPath.join(', ')}`
    });
  }
  ...
  if (existsSync(resolvedFilePath)) {
    return JSON.stringify({
      status: 'error',
      code: 'WRITE_CONFLICT',
      message: `Plik już istnieje: ${resolvedFilePath}`
    });
  }
...
async function start(): Promise<void> {
  ...
  const modelGateway = createModelGateway(
    async (toolCall: ToolCall, context?: ToolExecutionContext) => {
      ...
      const workspaceRoots = workspaceService.getActiveWorkspaceRoots();
    ...
    () => workspaceService.getActiveWorkspaceRoots()
  );
