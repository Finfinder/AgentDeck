...
describe('AgentRuntime Session Broker', () => {
  ...
  describe('createSession', () => {
    it('tworzy osobn─ů sesj─Ö dla ChatTab z w┼éasnym kontekstem, zakresem uprawnie┼ä i dziennikiem zdarze┼ä', () => {
    ...
    it('nie udost─Öpnia mutowalnego kontekstu mi─Ödzy sesjami ani snapshotami', () => {
      const { runtime } = createRuntime();
    ...
    it('nie pozwala mutowa─ç worker├│w i task├│w ze snapshot├│w', () => {
      const { runtime } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
  ...
  describe('worker lifecycle', () => {
    ...
    it('przepisuje narz─Ödzia u┼╝yte przez worker do zako┼äczonego taska', async () => {
      const { runtime, registerWorker, tick } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
    ...
    it('zg┼éasza b┼é─Ödny zakres przy pr├│bie u┼╝ycia tasku z innej sesji', () => {
      const { runtime } = createRuntime();
    ...
    it('zwraca kontrolowany b┼é─ůd gdy factory nie potrafi utworzy─ç worker', () => {
      const runtime = createAgentRuntime({
        workerFactory: () => {
  ...
  describe('crash and resume', () => {
    it('raportuje crash worker w dzienniku i udost─Öpnia token wznowienia', async () => {
      const { runtime, registerWorker } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
      ...
      registerWorker(worker.id, createFailingWorkerMock('B┼é─ůd narz─Ödzia'));
    ...
    it('wznawia crashed worker i czy┼Ťci poprzedni b┼é─ůd', async () => {
      const { runtime, registerWorker } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
      ...
      registerWorker(worker.id, createFailingWorkerMock('Poprzedni b┼é─ůd'));
      const runResult = runtime.runWorker(worker.id);
      expect(runResult.status).toBe('ok');
    ...
    it('ponawia transientny b┼é─ůd worker i ko┼äczy zadanie po sukcesie', async () => {
      const { runtime, registerWorker } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
  ...
  describe('subagent activity', () => {
    it('tworzy subagenta z ograniczonym zakresem uprawnie┼ä i zwraca task activity', async () => {
      const { runtime, registerWorker } = createRuntime();
      const session = expectOk(runtime.createSession({
        chatTabId: 'chat-tab-1',
        modelId: 'model-1',
      ...
      const subagent = expectOk(runtime.startSubagent({
        ...
        goal: 'Sprawd┼║ ryzyka bezpiecze┼ästwa',
        modelId: 'model-2',
    ...
    it('odmawia subagenta podpi─Ötego pod task z innej sesji', () => {
      const { runtime } = createRuntime();
