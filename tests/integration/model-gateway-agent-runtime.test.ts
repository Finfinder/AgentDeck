...
describe('ModelGateway ÔÇö AgentRuntime per ChatTab integration', () => {
  it('tworzy osobn─ů sesj─Ö runtime dla ka┼╝dego ChatTab', () => {
  ...
  it('przechowuje odr─Öbny model i permission scope dla ka┼╝dej sesji', () => {
    const gateway = new ModelGateway();
  ...
  it('aktualizuje model tylko w sesji powi─ůzanej z wybranym ChatTab', () => {
    const gateway = new ModelGateway();
  ...
  it('utrzymuje osobny event log dla ka┼╝dej sesji', async () => {
    const gateway = new ModelGateway();
    ...
    await gateway.sendMessage(firstTab.id, 'Cze┼Ť─ç z pierwszego czatu');
  ...
  it('uruchamia izolowanego worker runtime dla wiadomo┼Ťci z ChatTab', async () => {
    const gateway = new ModelGateway();
    gateway.registerAdapter(createMockAdapter('ollama', 'Runtime response'));

    const tab = gateway.createChatTab();
    const result = await gateway.sendMessage(tab.id, 'Cze┼Ť─ç');
  ...
  it('uruchamia subagenta przez ModelGateway z ograniczonym zakresem narz─Ödzi i zwraca wynik parentowi', async () => {
    ...
    const toolsForSubagent = vi.fn().mockImplementation(async function* () {
      ...
      yield { type: 'chunk', content: 'Zobacz [raport bezpiecze┼ästwa](docs/security.md) i [dokumentacj─Ö](https://docs.example.com/security).' };
    });
  ...
  it('zamyka tylko sesj─Ö runtime powi─ůzan─ů z zamykanym ChatTab', () => {
    const gateway = new ModelGateway();
  ...
  it('obs┼éuguje zdarzenia runtime emitowane przez ModelGateway', () => {
    const gateway = new ModelGateway();
    const handler = vi.fn();
