...
function formatInvalidToolArgumentsMessage(toolName: string): string {
  if (toolName === 'read_file') {
    return 'Nie mogę wykonać narzędzia `read_file`, bo model nie podał wymaganej ścieżki pliku. Poproś użytkownika o dokładną nazwę lub ścieżkę pliku.';
  }

  return `Nie mogę wykonać narzędzia \`${toolName}\`, bo model przekazał niepoprawne argumenty. Poproś użytkownika o dokładniejsze dane lub ścieżkę pliku.`;
}
